...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-cancel:hover { background: var(--slate-50); border-color: var(--navy-light); color: var(--navy); }
        .btn-save {
    ...
    <main class="main-content">
        <div class="main-wrap">
            ...
            <div class="pg-header">
                ...
                <button type="button" class="btn-save" data-bs-toggle="modal" data-bs-target="#createGroupModal" style="padding:0.5rem 0.75rem; font-size:0.78rem;">
                ...
                <div class="filter-grid">
                    ...
                    <div class="field">
                        ...
                        <select id="sortBy" class="field-input">
                            ...
                            <option value="debt">Remaining Debt</option>
                            <option value="badDebtor">Bad Debtor</option>
                        </select>
                    </div>
                     ...
                     @endif
                      ...
                      <div class="field">
                          ...
                          <div style="display:flex; gap:0.5rem; align-items:flex-end;">
                              <select id="filterGroup" class="field-input" style="flex:1;">
            ...
            <div id="selectionToolbar" class="selection-toolbar" style="display:none;">
                ...
                <button type="button" class="btn-save assign-group-btn" data-bs-toggle="modal" data-bs-target="#assignGroupModal">
                    <i class="bi bi-tags-fill"></i> Add to Group
                </button>
                <button type="button" class="btn-cancel clear-selection-btn" onclick="clearSelection()">Cancel</button>
            </div>
            ...
            <div class="panel">
                <div class="table-wrap">
                    <table class="cust-tbl" id="customersTable">
                        <thead>
                            <tr>
                                <th style="width:4%;"><input type="checkbox" id="selectAll" title="Select All"></th>
                                <th style="width:4%;">#</th>
                                <th style="width:14%;" class="sortable" data-sort="name">Name</th>
                                <th style="width:10%;" class="sortable" data-sort="group">Group</th>
                                ...
                                <th style="width:11%;" class="sortable" data-sort="debt">Debt</th>
                                <th style="width:11%;" class="sortable" data-sort="badDebtor">Bad Debtor</th>
                                <th style="width:11%;" class="sortable" data-sort="date">Member Since</th>
                                <th style="width:14%;" class="sortable" data-sort="account">Account</th>
                        ...
                        <tbody id="customersBody">
                            ...
                            @else
                                ...
                                <tr class="customer-row"
                                    ...
                                    data-group="{{ strtolower($customer->groups ?? '') }}"
                                    data-business="{{ strtolower($customer->business) }}"
                                    data-credit="{{ floatval($customer->limits) }}"
                                    data-debt="{{ floatval($remainingDebts[$customer->id] ?? 0) }}"
                                    data-bad-debtor="{{ $badDebtorDays[$customer->id] ?? '' }}"
                                    data-date="{{ strtotime($customer->created_at) }}"
                                    data-account="{{ strtolower($customer->account ?? '') }}">
                                    
                                    <td><input type="checkbox" class="customer-select" value="{{ $customer->id }}"></td>
                                    <td>{{ $index + 1 }}</td>
                                    <td><div class="cust-name">{{ $customer->name }}</div></td>
                                    <td>
                                        <span class="account-badge">
                                        {{ $customer->groups ?  e($customer->groups)  : '—' }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="credit-amt">Tsh {{ number_format($remainingDebts[$customer->id] ?? 0, 0) }}</span>
                                    </td>
                                    <td>
                                        @php
                                            $days = $badDebtorDays[$customer->id] ?? null;
                                        @endphp
                                        @if($days === null)
                                            <span style="color: var(--slate-400);">—</span>
                                        @elseif($days < 0)
                                            <span style="color: #E11D48; font-weight: 600;">Overdue ({{ abs($days) }}d)</span>
                                        @else
                                            <span style="color: var(--emerald); font-weight: 500;">{{ $days }}d left</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @php
                                        $name = DB::table('accounts')->where('id', $customer->account)->value('name');
                                        @endphp
                                        <span class="account-badge">{{ $name ?? '—' }}</span>
                                    </td>
                                    <td>
                                        <div class="action-btns">
                                            <a href="{{ url('customerView') }}?id={{ urlencode($customer->id) }}" class="act-btn act-btn-view"><i class="bi bi-eye"></i> View</a>
            ...
            <div class="modal-body">
                <form action="newCustomer" method="post">
                    ...
                    <div class="mfield">
                        ...
                        <div class="mfield">
                            <label class="mfield-label">Group</label>
                            <select name="groups" class="mfield-input" id="customerGroupSelect">
                    ...
                    <div class="row-fields">
                        ...
                        <div class="mfield">
                            <label class="mfield-label">Due date</label>
                            <div class="input-grp">
                                <span class="input-grp-text">Days</span>
                                <input type="number" name="due" placeholder="Due date" min="0" max="30" required>
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="createGroupModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header-navy">
                <h4 class="modal-title">
                    <i class="bi bi-tags-fill"></i> Create New Group
                </h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="createGroupForm">
                    @csrf
                    <div class="mfield">
                        <label class="mfield-label">Group Name</label>
                        <input type="text" class="mfield-input" id="groupNameInput" placeholder="e.g. VIP, Wholesale, Regular" required>
                    </div>
                </form>
            </div>
            <div class="modal-footer-custom">
                <button type="button" class="btn-cancel" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn-save" id="saveGroupBtn">
...
<div class="modal fade" id="assignGroupModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header-navy">
                <h4 class="modal-title">
                    <i class="bi bi-tags-fill"></i> Assign to Group
                </h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="assignGroupForm" method="POST" action="{{ url('customers/bulk-group') }}">
                @csrf
                <div class="modal-body">
                    <div class="mfield">
                        <label class="mfield-label">Group Name</label>
                        <select name="group" class="mfield-input assign-group-select" required>
                            <option value="">Select group</option>
                            @if(isset($groups) && count($groups) > 0)
                                @foreach($groups as $group)
                                    <option value="{{ $group }}">{{ $group }}</option>
                    ...
                    <div id="bulkCustomerIdsContainer"></div>
                </div>
                <div class="modal-footer-custom">
                    <button type="button" class="btn-cancel" data-bs-dismiss="modal">Cancel</button>
...
<script>
    ...
    function sortTable() {
        ...
        rows.sort((a, b) => {
            ...
            if (currentSort.field === 'sales' || currentSort.field === 'credit' || currentSort.field === 'date' || currentSort.field === 'debt' || currentSort.field === 'badDebtor') {
    ...
    function filterTable() {
        ...
        rows.forEach(row => {
            ...
            row.style.display = (matchesSearch && matchesBusiness && matchesAccount && matchesGroup) ? '' : 'none';
        });
    }
    ...
    document.getElementById('customerGroupSelect')?.addEventListener('change', function() {
    ...
    document.getElementById('saveGroupBtn')?.addEventListener('click', async function() {
        ...
        try {
            ...
            const customerSelect = document.getElementById('customerGroupSelect');
            if (customerSelect) {
                const createOption = customerSelect.querySelector('option[value="__create_new__"]');
                customerSelect.insertBefore(option, createOption);
            }

            const filterSelect = document.getElementById('filterGroup');
            const assignSelect = document.querySelector('.assign-group-select');
            if (filterSelect) {
            ...
            if (assignSelect) {
                ...
                aOpt.value = name;
                aOpt.textContent = name;
                const createOption = assignSelect.querySelector('option[value="__create_new__"]');
                assignSelect.insertBefore(aOpt, createOption);
            }
            ...
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-check-lg"></i> Create Group';
        }
    });

    document.querySelector('.assign-group-select')?.addEventListener('change', function() {
        if (this.value === '__create_new__') {
            const modal = new bootstrap.Modal(document.getElementById('createGroupModal'));
            modal.show();
            this.value = '';
    ...
    function updateSelectionUI() {
        const checked = Array.from(customerCheckboxes).filter(cb => cb.checked);
    ...
    if (assignGroupForm) {
        assignGroupForm.addEventListener('submit', function(e) {
            ...
            const container = document.getElementById('bulkCustomerIdsContainer');
            container.innerHTML = '';
            checked.forEach(cb => {
