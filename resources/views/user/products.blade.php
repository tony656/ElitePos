    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <!-- Delete Offer Button (shown when editing existing offer) -->
...
<script>
// [JavaScript from original - keeping core functionality intact]
...
function generatePrintContent(products) {
    ...
    const rows = products.map((p, i) => `
        <tr>
            ...
            <td style="width:40%">
                <div class="prod-disp-name">${p.name1}</div>
                <div class="prod-disp-sub">${p.name2}</div>
            </td>
            <td style="width:20%">${p.category}</td>
...
function openDeleteModal() {
    ...
    if (selected.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:var(--slate-400);padding:1rem;">No products selected</p>';
    } else {
        selected.forEach(p => {
...
// Global variable to store current product ID for offer
let currentOfferProductId = null;
let currentOfferId = null;

function openOfferModal(productId = null, productName = '') {
    console.log('openOfferModal called with:', { productId, productName });
    ...
    if (productId) {
        console.log('✅ Setting product info:', productName || 'Unknown', productId);
        $('#offerProductName').text(productName || 'Unknown Product');
        $('#offerProductId').text('ID: ' + productId);
        $('#offerProductIdInput').val(productId);
        $('#existingOffersSection').show();
        
        // Load existing offers for this product
        loadOffersForProduct(productId);
    } else {
        console.log('❌ No product ID - showing placeholder');
        $('#offerProductName').text('Select a product first');
        $('#offerProductId').text('—');
        $('#offerProductIdInput').val('');
        $('#existingOffersSection').hide();
        $('#offersList').html('<p style="text-align:center;color:var(--slate-400);padding:1rem;">Select a product to view and create offers</p>');
    }
    
    // Reset form
    $('#offerForm')[0].reset();
    $('#offerFormMessage').html('');
    $('#deleteOfferSection').hide();
    
    new bootstrap.Modal(document.getElementById('offerModal')).show();
}

function loadOffersForProduct(productId) {
    console.log('Loading offers for product ID:', productId);
    $.ajax({
        ...
        error: function(xhr) {
            ...
            $('#offersList').html('<p style="text-align:center;color:var(--rose);padding:1rem;">Error loading offers: ' + xhr.status + '</p>');
        }
    });
}

function displayOffersList(offers) {
    console.log('Displaying offers:', offers);
    const container = $('#offersList');
    container.empty();
    
    if (!offers || offers.length === 0) {
        container.html('<p style="text-align:center;color:var(--slate-400);padding:1rem;">No active offers for this product</p>');
        return;
    }
    
    offers.forEach(offer => {
        console.log('Offer:', offer, 'offeredProduct:', offer.offeredProduct);
        ...
        const item = $(`
            <div class="selected-item" data-offer-id="${offer.id}">
                <div>
                    <div class="selected-item-name">${offerText}</div>
                    <div style="font-size:0.75rem;color:var(--slate-400);">
                        ${productName} in stock
                    </div>
                </div>
                <button class="mbtn mbtn-outline" style="padding:0.25rem 0.5rem;font-size:0.75rem;"
                        onclick="editOffer(${offer.id})" title="Edit">
...
function editOffer(offerId) {
    // Find the offer data from the current list
    const offerItem = $(`.selected-item[data-offer-id="${offerId}"]`);
    const offerText = offerItem.find('.selected-item-name').text();
    
    // Parse "Buy X get Y" format
    const match = offerText.match(/Buy (\d+) × get (\d+) ×/);
    if (match) {
        $('#requiredQuantity').val(match[1]);
        $('#offerQuantity').val(match[2]);
        currentOfferId = offerId;
        $('#deleteOfferSection').show();
        
        // Scroll to form
        $('html, body').animate({ scrollTop: $('#offerForm').offset().top - 100 }, 300);
    }
}
...
// Update the Offers button click handlers
$(document).ready(function() {
    // Override the original openOfferModal to accept parameters
    window.openOfferModal = openOfferModal;
    
    // Handle header Offers button - uses selected checkbox product
    $('#offerBtn').on('click', function(e) {
        e.preventDefault();
        const selectedProduct = $('.product-checkbox.checked').closest('tr').first();
        if (selectedProduct.length) {
            const productId = selectedProduct.data('product-id');
            const productName = selectedProduct.find('.prod-name').text().trim();
            openOfferModal(productId, productName);
        } else {
            alert('Please select at least one product to manage offers');
        }
    });
    
    console.log('Offer modal ready - header button handler set. Row buttons use inline onclick.');
    
    // Handle Offers button from search panel
...
function updateOfferBadgeInTable(productId, hasOffer) {
    ...
    const nameCell = row.find('.prod-name');
    
    if (hasOffer) {
        if (nameCell.find('.offer-tag').length === 0) {
            nameCell.append(`
                <span class="offer-tag" title="Active offer">
                    <i class="bi bi-gift-fill"></i> Offer
                </span>
