...
<button class="btn bg-color" onclick="downloadReport()">
...
<script>
    function downloadReport() {
        ...
        const url = `/export-monthly-report?month=${currentMonth}`;
    ...
    <div class="btn-group w-100 my-3">
        <button class="btn bg-primary bg-opacity-75 rounded-4 p-3 text-light stylish-btn rounded-4">
...
{{(number_format($Tsale))}}
    ...
    <button class="btn bg-warning bg-opacity-75 mx-3 rounded-4 p-3 text-light  stylish-btn rounded-4">
                <div class="container text-start">
                    <h6>
                       Discount made
                    </h6>
    ...
    {{(number_format($Tdiscount))}}
                ...
                <div class="container d-flex justify-content-between">
                    ...
                    <h6>
                        {{(number_format($Mdiscount))}}
                    </h6>
                </div>
            </button>

            <button class="btn bg-success bg-opacity-75 rounded-4 p-3 text-light  stylish-btn rounded-4" >
                        <div class="container text-start">
                            <h6>
                               Sold Product
                            </h6>
            ...
            @endphp      
            <span>
                <svg width="60px" height="60px" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" version="1.1" fill="none" stroke="#000000" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="m4.25 6.75-2.5 1.25 6.25 3.25 6.25-3.25-2.5-1.25m-10 4.25 6.25 3.25 6.25-3.25"></path> <path d="m8 8.25v-6.5m-2.25 2 2.25-2 2.25 2"></path> </g></svg>
            </span>
            <h3>
            {{(number_format($Tproduct))}}
            </h2>
                        </div>
                        <div class="container d-flex justify-content-between">
    ...
    <div class="row">
        <div class="col">
            ...
            <div class="container p-3">
                ...
                <input type="search" id="search-input" class="form-control rounded-5" placeholder="Search sale name"> 
               </div>
            <div class="table">
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="viewSales" method="post">
                                    @csrf
                                    <button class="btn btn-sm bg-color" name="salesName" value="{{$sale->salesName}}">
                                        <i class="bi bi-eye"></i>
                                        View
                                    </button>
                                  </form>
    ...
    font-weight: bold;
            ...
            <div id="calendar-navigation" class="container-fluid w-100 d-flex justify-content-between">
                <button id="prevMonth" class="btn bg-color">
                    <i class="bi bi-caret-left-fill"></i>
                </button>
                <h5 id="currentMonthYear" class="text-center"></h5>
                <button id="nextMonth"  class="btn bg-color">
            ...
            <form id="dateForm" action="{{ route('user.saleDate') }}" method="post">
                @csrf
                <input type="hidden" id="selectedDate" name="selectedDate" onchange="this.form.submit()">
            </form>
            <div class="container bg-light p-3">
              <h5>
                Total Sales
              {{(number_format($sales->sum('amount') ?? 0))}}
              /=
              </h5>
            </div>
