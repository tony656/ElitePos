...
<body>
            ...
            <div class="main-wrap">
                ...
                <div class="pg-header">
                    ...
                    <div class="header-actions">
                        ...
                        <a href="{{ url('admin/make-return') }}" class="hbtn hbtn-outline-rose">
                            <i class="bi bi-arrow-return-left"></i> Make Return
                        </a>
                    </div>
                </div>
                ...
                <div class="layout">
                    ...
                    <div class="panel">
                        <div class="panel-head">
                            <i class="bi bi-receipt"></i> Receiving Details
                        </div>

                        {{-- Form fields --}}
                        <div class="rp-section" style="flex:1; overflow-y:auto;">
                            <form id="orderForm" action="{{ route('admin.process-receiving') }}" method="POST">
                                @csrf
                                ...
                                <div class="field-grid" style="margin-bottom:0.65rem;">
                                    <div class="field">
                                        ...
                                        <select name="supplier" id="supplier" class="field-input" required onchange="loadSupplierUsers()">
                                            ...
                                            @foreach (DB::table('vendors')->where('account', session('account_id'))->get() as $vendor)
                                    ...
                                    <div class="field">
                                        ...
                                        <select name="served" id="served" class="field-input" required onchange="loadUserSuppliers()">
                                            ...
                                            @php
                                                // Get all accessible users based on user's assigned accounts
                        ...
                        <div class="rp-section">
                            ...
                            <div class="action-btns">
                                ...
                                <button type="button" class="abtn abtn-save" id="submitBtn">
                                    <i class="bi bi-check-circle-fill"></i> Save Receiving
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    function loadUserSuppliers() {
        ...
        fetch('{{ route("user.get-user-suppliers") }}?user_id=' + userId, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(r => r.json())
        .then(data => {
            if (data.success && data.suppliers && data.suppliers.length > 0) {
                ...
                allocatedList.innerHTML = '<div style="color: var(--slate-500); font-style: italic;">No suppliers assigned to you</div>';
                allocatedSection.style.display = 'block';
            }
        })
        .catch(err => {
    ...
    function loadSupplierUsers() {
        ...
        fetch('{{ route("user.get-supplier-users") }}?supplier_id=' + supplierId, {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(r => r.json())
        .then(data => {
    ...
    supplierSelect?.addEventListener('change', async function() {
        ...
        try {
            const response = await fetch(`/user/get-supplier-users?supplier_id=${supplierId}`);
    ...
    servedSelect?.addEventListener('change', async function() {
        ...
        try {
            const response = await fetch(`/user/get-user-suppliers?user_id=${userId}`);
            const data = await response.json();
