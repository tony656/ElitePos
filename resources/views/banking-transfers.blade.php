...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .search-item:hover { background: var(--slate-50); border-left: 3px solid var(--amber); padding-left: 10px; }
        .search-item-none { padding: 12px; text-align: center; font-size: 13px; color: var(--slate-500); }
                                    ...
                                    <tr>
                                        ...
                                        <td data-label="Actions">
                                            <div class="action-row">
                                                ...
                                                <form action="/banking-transfer/delete/{{ $transfer->id }}" 
                                                    method="POST" 
...
<div class="modal fade" id="addBankingTransfer" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="/banking-transfer/store" method="post">
                    @csrf
  ...
  <div class="form-group">
                             ...
                             <div class="search-wrap">
                                 ...
                                 <input type="text" class="form-control search-input" id="supplierSearch"
                                     placeholder="Search supplier…" autocomplete="off" required>
                             </div>
                             <input type="hidden" name="supplier_id" id="supplierId">
                             <div class="form-hint">Start typing to search for a supplier</div>
                         ...
                         <div class="form-group">
                             ...
                             <div class="search-wrap">
                                 <i class="bi bi-search"></i>
                                 <input type="text" class="form-control search-input" id="beneficiarySearch"
                                     placeholder="Search beneficiary…" autocomplete="off" required>
                             </div>
                             <input type="hidden" name="beneficiary_id" id="beneficiaryId">
                             <div class="form-hint">Start typing to search for a beneficiary</div>
...
$(document).ready(function() {
    ...
    $('#supplierSearch').on('input', function() {
        ...
        $.get('{{ url("banking/search-supplier") }}', { query: query }, function(data) {
            if (data.length === 0) {
                $results.html('<div class="search-item-none">No suppliers found</div>').show();
            } else {
                $results.html(data.map(function(s) {
                    return '<div class="search-item" data-id="' + s.id + '">' + s.name + '</div>';
                }).join('')).show();
            }
        }).fail(function() {
            $results.html('<div class="search-item-none" style="color:var(--rose);">Error loading suppliers</div>').show();
        });
    });

    // Select supplier from results
    $(document).on('click', '#supplierResults .search-item', function() {
        var id = $(this).data('id');
        var name = $(this).text();
        $('#supplierId').val(id);
        $('#supplierSearch').val(name);
        $('#supplierResults').empty().hide();

        var supplier = suppliersData.find(function(s) { return s.id == id; });
        if (supplier) {
            populateAccountDropdown($('#supplier_account_id'), supplier.accounts);
        }
    });
    ...
    $('#beneficiarySearch').on('input', function() {
        var query = $(this).val().trim();
        var $results = $('#beneficiaryResults');
        ...
        $.get('{{ url("banking/search-beneficiary") }}', { query: query }, function(data) {
            if (data.length === 0) {
                $results.html('<div class="search-item-none">No beneficiaries found</div>').show();
            } else {
                $results.html(data.map(function(b) {
                    return '<div class="search-item" data-id="' + b.id + '">' + b.name + '</div>';
                }).join('')).show();
            }
        }).fail(function() {
            $results.html('<div class="search-item-none" style="color:var(--rose);">Error loading beneficiaries</div>').show();
        });
    });

    // Select beneficiary from results
    $(document).on('click', '#beneficiaryResults .search-item', function() {
        var id = $(this).data('id');
        var name = $(this).text();
        $('#beneficiaryId').val(id);
        $('#beneficiarySearch').val(name);
        $('#beneficiaryResults').empty().hide();

        var beneficiary = beneficiariesData.find(function(b) { return b.id == id; });
        if (beneficiary) {
            populateAccountDropdown($('#beneficiary_account_id'), beneficiary.accounts);
        }
    });
    ...
    $(document).on('click', function(e) {
        ...
        if (!$(e.target).closest('#beneficiarySearch, #beneficiaryResults').length) {
            $('#beneficiaryResults').empty().hide();
        }
    });
