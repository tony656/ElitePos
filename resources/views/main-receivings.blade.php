...
<body>
            ...
            <div class="main-wrap">
                ...
                <div class="pg-header">
                    <div class="pg-title">
                        ...
                        {{ __('messages.main_store_receivings_page_header') }}
                    </div>
                ...
                <div class="card-panel">
                    ...
                    <div class="toolbar">
                        <div class="toolbar-left">
                            ...
                            <div class="toolbar-label">
                                ...
                                {{ __('messages.receivings_list_non_returns') }}
                            </div>
                        </div>
                        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;align-items:center;">
                            <form method="POST" action="{{ route('approve-all-receivings') }}">
                                @csrf
                    ...
                    <div class="summary-bar">
                        ...
                        <div class="summary-divider"></div>
                        <div class="summary-stat">
                            <span class="summary-stat-label">Total Value - Return</span>
                            <span class="summary-stat-value" style="color:var(--rose);">Tsh {{ number_format($totalReturnValue, 2) }}</span>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    ...
                                    <tr id="row-{{ $item->productId }}" class="{{ $item->status == 'Approved' ? 'row-approved' : '' }}">
                                        ...
                                        <td>
                                         ...
                                         <td>
                                            @php
                                                $retQty = $returnQtyMap[$item->productId] ?? 0;
                                            @endphp
                                            @if($retQty > 0)
                                                <span class="mono amount" style="color:var(--rose);font-weight:600;">{{ number_format($retQty) }}</span>
                                            @else
                                                <span class="mono" style="color:var(--slate-400);">—</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($item->isDebt == 1)
                                        ...
                                        <td>
                                            @php
                                                $supplierName = DB::table('accounts')->where('id', $item->supplier)->value('name');
                                            @endphp
                                            <div class="person-cell">
                                                <div class="person-avatar">{{ substr($supplierName ?? 'U', 0, 2) }}</div>
                                                {{ $supplierName ?? 'Unknown Supplier' }}
                                            </div>
                                        </td>
                                        <td>
                                            @php
                                                $allocatedName = DB::table('accounts')->where('id', $item->account)->value('name');
                                            @endphp
                                            <div class="person-cell">
                                                <div class="person-avatar" style="background:#E0E7FF;color:#3730A3;">{{ substr($allocatedName ?? 'U', 0, 2) }}</div>
                                                {{ $allocatedName ?? 'Unknown' }}
                                            </div>
                                        </td>
                                        <td>
                                            <div class="action-wrap">
                                                <form method="post">
                                                    ...
                                                    <button formaction="{{ url('dltrestock') }}" class="act-btn act-delete" title="Delete"
                                                ...
                                                <form method="post">
                                                    ...
                                                    <input type="hidden" name="shop" value="{{ $shopFilter ?? '' }}">
                                                    <button formaction="{{ url('restockProd') }}" class="act-btn act-approve" title="Approve">
                                                ...
                                                <form method="post">
                                                    ...
                                                    <button formaction="{{ url('undo-receivings') }}" class="act-btn act-undo" title="Undo"
    ...
    document.getElementById('undoForm').addEventListener('submit', function(e) {
        const checked = document.querySelectorAll('.item-checkbox:checked');
        if (!checked.length) { e.preventDefault(); alert('Select at least one item to undo.'); return; }
        collectSelected(this);
    });
