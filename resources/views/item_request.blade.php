...
<head>
    ...
    <title>{{config("app.name")}} — {{ __('messages.item_request_page_title') }}</title>
    @include("links")
...
#search-results {
    ...
    left: 0;
    right: 0;
    z-index: 2;  /* Increased from 2 */
    background: var(--white);
    border: 1.5px solid var(--navy-light);
    border-radius: var(--radius);
    max-height: 700px;  /* Reduced from 700px for better UX */
    overflow-y: auto;
    ...
    display: none;
        ...
        .search-item-price { font-size: 12px; color: var(--emerald);  font-weight: 500; margin-top: 2px; }
        ...
        .prod-id   { font-size: 11px; color: var(--slate-400);  }
        ...
        .price-val {  font-size: 12.5px; font-weight: 500; color: var(--emerald); }
        .total-val {  font-size: 13px;   font-weight: 600; color: var(--slate-800); }
        ...
        .info-val   { font-weight: 600; color: var(--slate-800);  font-size: 12.5px; }
        ...
        .price-row .price-num {  font-weight: 600; color: var(--amber); }
        ...
        <div class="page-header fade-up fade-up-1">
            <div class="header-title">
                <div class="header-icon">
                    ...
                    <div>
                        <div class="header-label">{{ __('messages.item_request_page_title') }}</div>
                        <div class="header-sub">{{ date('l, d M Y') }}</div>
                    </div>
                </div>
                <a href="{{ url('viewRequest') }}" class="btn-header">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/>
                        <line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/>
                        <line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/>
                    </svg>
                    {{ __('messages.view_all_requests') }}
                </a>
        </div>
            ...
            <div style="display:flex; flex-direction:column; gap:1.25rem;">
                ...
                <div class="card fade-up fade-up-2">
                    <div class="card-header">
                        ...
                        <span class="card-title">{{ __('messages.add_new_item') }}</span>
                    </div>
                    <div class="card-body">
                            ...
                            @csrf
                                <div class="form-group" style="margin-bottom:0;">
                                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px;">
                                        <label class="form-label" style="margin-bottom:0;">{{ __('messages.select_shop') }}</label>
                                    </div>
                                    <select class="form-select" name="shopId" id="shopId" onchange="this.form.submit()">
                                        <option value="">— {{ __('messages.choose_shop') }} —</option>
                        ...
                        <form action="/itemRequest" method="post" autocomplete="off" id="addItemForm">
                            @csrf
                            <input type="hidden" name="OrderName" value="{{ $orders->orderName ?? '' }}">
                                
                            {{-- Product search --}}
                            <div class="form-group search-wrap">
                                <label class="form-label">{{ __('messages.search_product') }}</label>
                                <input type="search" class="form-control" id="product-name"
                                       placeholder="{{ __('messages.type_product_name') }}" autocomplete="off">
                ...
                {{-- Items table --}}
                <div class="card z fade-up fade-up-3">
                    <div class="card-header">
                        ...
                        <span class="card-title">{{ __('messages.current_items') }}
                            @if(count($carts) > 0)
                    ...
                    <div class="card-body" style="padding: 0;">
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('messages.col_qty') }}</th>
                                        <th style="text-align:right;">{{ __('messages.col_unit_price') }}</th>
                                        <th style="text-align:right;">{{ __('messages.col_subtotal') }}</th>
                                        <th style="text-align:right;">{{ __('messages.col_del') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="prod-name">{{ $name ?? __('messages.product_not_found') }}</div>
                                        </td>
                                        <td>
                                            <form action="/requpdQuant" method="post">
                                                @csrf
                                        ...
                                        <td class="text-right">
                                            <form action="/dltItemReq" method="post" style="display:inline;">
                                                @csrf
                                                ...
                                                <button type="submit" class="btn-del"
                                                        onclick="return confirm('{{ __('messages.remove_this_item') }}')">
                                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="3" class="tfoot-label">{{ __('messages.grand_total') }}</td>
                        ...
                        <div class="empty-state" style="margin: 1.25rem;">
                            ...
                            <h5>{{ __('messages.no_items_yet') }}</h5>
                            <p>{{ __('messages.search_add_products_begin') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="fade-up fade-up-3">
                <div class="card">
                    <div class="card-header">
                        ...
                        <span class="card-title">{{ __('messages.request_summary') }}</span>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="order-info">
                            <div class="order-info-row">
                                <span class="info-label">{{ __('messages.request_id') }}</span>
                            ...
                            <div class="order-info-row">
                                <span class="info-label">{{ __('messages.served_by') }}</span>
                        ...
                        {{-- Supplier --}}
                            <div style="margin-bottom:1.25rem;">
                                <div class="section-label">{{ __('messages.supplier') }}</div>
                            <form action="/saveInfo" method="post">
                                @csrf
                                <input type="hidden" name="requestName" value="{{ $orders->requestName ?? 'NEW-ORDER' }}">
                                <div class="form-group" style="margin-bottom:0;">
                                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px;">
                                        <label class="form-label" style="margin-bottom:0;">{{ __('messages.select_supplier') }}</label>
                                        <span style="font-size:12px; color:var(--slate-500);">{{ __('messages.current') }} <strong>{{ $supplierName }}</strong></span>
                                    </div>
                                    <select class="form-select" value="{{ $orders->supplierId ?? '' }}" name="selectedCustomer" onchange="this.form.submit()">
                                        <option value="" selected disabled>{{ __('messages.choose_supplier') }}</option>
                            ...
                            <div class="form-group">
                                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:6px;">
                                        <label class="form-label" for="assignedTo">{{ __('messages.assign_to') }}</label>
                                         <span style="font-size:12px; color:var(--slate-500);">{{ __('messages.current') }} <strong>{{ $Allocation }}</strong></span>
                                    </div>
                              
                                    </div>
                                    <select class="form-select" name="assignedTo" id="assignedTo" onchange="this.form.submit()">
                                        <option value="">— {{ __('messages.select_user_allocation') }}</option>
                                ...
                                <p class="form-hint">{{ __('messages.select_user_location_request') }}</p>
                            </div>
                            </form>
                        </div>
                        ...
                        <form action="/requestSubmit" method="POST">
                            @csrf
                                <div style="margin-bottom:1.25rem;">
                            <div class="section-label">{{ __('messages.request_shop') }}</div>
   
                                <input type="hidden" name="requestName" value="{{ $orders->requestName ?? 'NEW-ORDER' }}">
                            
                        </div>
                                @if (canUser('item_request_date'))
                                <div class="form-group">
                                <label class="form-label" for="requestDate">{{ __('messages.request_date') }}</label>
                        ...
                        <div class="alert alert-info" style="margin:0;">
                            <span>{{ __('messages.add_products_begin_request') }}</span>
                        </div>
                        @endif
...
$(document).ready(function () {
    ...
    $('#product-name').on('input', function () {
        ...
        if (query.length > 1) {
            $.ajax({
                url: "{{ url('requestSearch') }}",
                method: 'GET',
                data: { query: query },
                success: function (data) {
                    if (!data || data.error) {
                        $('#search-results')
                            .html('<div class="search-no-results">' + (data.error || '{{ __('messages.search_no_results') }}') + '</div>')
                            .show();
                        return;
                    }
                    let output = '';
                    data.forEach(function (product) {
                        ...
                        output += `
                            <div class="search-item"
                                ...
                                <div class="search-item-quantity float-end">${product.quantity}</div>
                ...
                error: function (xhr) {
                    let msg = '{{ __('messages.error_loading_results') }}';
                    if (xhr.responseJSON && xhr.responseJSON.error) {
                        msg = xhr.responseJSON.error;
                    } else if (xhr.status === 401) {
                        msg = '{{ __('messages.session_expired') }}';
                    } else if (xhr.status === 0) {
                        msg = '{{ __('messages.cannot_connect') }}';
                    }
                    $('#search-results')
